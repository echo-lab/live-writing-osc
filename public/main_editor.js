...
// https://css-tricks.com/snippets/javascript/javascript-keycodes/
    ...
    var shiftLetterHorizontallyCodeMirror = function(line,ch, shiftAmount){
      ...
      var localOffset = offset * (1+sizeFactor*2.0);
      geo[geoindex].vertices[strIndex*4].x = (ch+shiftAmount) * scaleX;
      geo[geoindex].vertices[strIndex*4+1].x = (ch+shiftAmount) * scaleX+localOffset;
      geo[geoindex].vertices[strIndex*4+2].x = (ch+shiftAmount) * scaleX+localOffset;
      geo[geoindex].vertices[strIndex*4+3].x = (ch+shiftAmount) * scaleX;
    }
    ...
    var changeCodeMirrorFunc = function(instance, change){
      ...
      if(added){
        ...
        if(change.text.length == 1){
          ...
          // the leftover in last line shifting
          for (var i=startCh; i<cmGrid[startLine].length; i++){
            if(change.text.length>1)
